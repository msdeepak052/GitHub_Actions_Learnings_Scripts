...
Let's say you have:
1. A **test workflow** that runs tests on your code
2. A **deploy workflow** that should only run if the tests pass
...
on:
  workflow_run:
    workflows: ["Run Tests"]  # Must match the name of the test workflow
    types:
      - completed
    branches: [ main ]
